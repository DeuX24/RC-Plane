...
static const int SPI_Frequency =
    4000000; // Stable even with a long jumper cable
...
void Nrf24_writeAckPayload(NRF24_t * dev, uint8_t pipe, uint8_t * payload, uint8_t len) {
    uint8_t cmd = 0xA8 | (pipe & 0x07); // W_ACK_PAYLOAD command for the specific pipe
    ...
    spi_device_transmit(dev->_SPIHandle, &t1);

    // Write the payload to the FIFO
    if (len > 0) {
        spi_transaction_t t2 = { .length = len * 8, .tx_buffer = payload };
        spi_device_transmit(dev->_SPIHandle, &t2);
    }
